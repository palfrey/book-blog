...
earlierPattern = compile(
    "<a href='([^']+)'>.+?Earlier Stories.+?</a>", DOTALL | MULTILINE
)
titlePattern = compile("<h2>(.+?)</h2>")
subtitlePattern = compile('<p class="standfirst">(.+?)</p>')
contentPattern = compile(
    '<strong class="trailer">.+?</p>(.+?)(?:(?:<p>(?:(?:<i>)|(?:<small>)|(?:<font size="-2">)|(?:<br>\n))?BOFH .+? Simon Travaglia)|(?:<ul class="noindent">)|(?:<ul>.+?<li><a href="http://www.theregister.co.uk/content/30/index.html">BOFH: The whole shebang</a></li>)|(?:</form>))',
    DOTALL | MULTILINE,
)
