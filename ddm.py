...
contentPattern = compile(
    '(?:<BR>\s+<BLOCKQUOTE>|<H3 ALIGN="CENTER">)(.+)</BLOCKQUOTE>.+?<A HREF="ancilpag.html#DP">Dramatis personae</A>',
    DOTALL | MULTILINE,
)  # <A HREF=\"[^\"]+\"><IMG SRC=\"graphics/j-gate.jpg\" ALIGN=\"BOTTOM\" BORDER=\"0\" ALT=\"Into jump gate\"></A>" , DOTALL|MULTILINE)
nextPattern = compile(
    '<A HREF="([^"]+)"><IMG SRC="graphics/j-gate.jpg" ALIGN="BOTTOM" BORDER="0" ALT="Into jump gate"></A>'
)

volumePattern = compile(
    '<A HREF="(v\dcont.html)"><IMG HEIGHT="\d+" WIDTH="\d+" SRC="graphics/[^"]+" ALIGN="[^"]+" BORDER="0" ALT="(Volume \d - [^"]+)">'
)
