...
titlePattern = compile(
    '<span class="titre_story">\s+(.+?)\s+</span>', MULTILINE | UNICODE
)
contentPattern = compile(
    '<td class="bandeau_td_2" [^>]+>(.+?)<span class="texte_comment_blanc">',
    DOTALL | MULTILINE,
)

chapterPattern = compile("(.*?) - Chapter ([^ ]+) - (.+)")
memberPattern = compile(
    '<a class="link_page_stories" href="(/profile\d+/[^"]+)">([^<]+)</a>'
)
chapterLinkPattern = compile(
    'class="link_stories" href="(/story/\d+/[^"]+)">([^<]+)</a>'
)
...
memberData = cache.get(
    urljoin(url, memberPage.groups()[0].replace("&amp;", "&")), max_age=3600
).read()
