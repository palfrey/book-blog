...
toc = tocStart(folder)
content_pattern = re.compile("<div id=\"page-content\">(.+)</div>", re.MULTILINE|re.DOTALL)
...
for (link, title) in links:
    if link.startswith("http") or link.startswith("/scp-") or link.startswith("/system:") or link.endswith("author-page"):
        continue
