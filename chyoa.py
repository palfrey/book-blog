...
authorPattern = re.compile(
    'Created on <strong>[^<]+</strong>\nby <a href="https://chyoa.com/user/.+">(.+)</a>',
    re.MULTILINE,
)
contentPattern = re.compile(
    '<div class="chapter-content">(.+)</div>\s+<div class="chapter-options">',
    re.DOTALL | re.MULTILINE,
)
questionHeaderPattern = re.compile(
    '<header class="question-header">\n<h2>(.+)</h2>', re.DOTALL | re.MULTILINE
)
questionPattern = re.compile(
    '<li><a href="(https://chyoa.com/chapter/.+.\d+)" class="">(.+)</a></li>'
)
...
while len(todo) > 0:
    ...
    for questionURL, question in questions:
        content += (
            f'<li><a href="{hexdigest_md5(questionURL)}.html">{question}</a></li>\n'
        )
